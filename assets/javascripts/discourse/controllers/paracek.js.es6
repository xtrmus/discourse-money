...
export default Ember.Controller.extend({
  ...
  actions: {
    createPayment(adsoyad,paramiktari,ibanpapara,adres,durum,zaman) {
		...
		if(isNaN(parseInt(this.currentUser.custom_fields.money_value))){
			alert("Kayıt yapılamadı. Sorun olduğunu düşünüyorsanız lütfen yönetici ile görüşün");
			return;
		}
	  ...
	  if( parseInt(this.currentUser.custom_fields.money_value) < parseInt(paramiktari)){
	  ...
	  var yeniPara = parseInt(this.currentUser.custom_fields.money_value) - parseInt(paramiktari)
	  

      paymentRecord.save()
        .then(result => {
          this.payments.pushObject(result.target);
			this.currentUser.set("custom_fields.money_value",yeniPara);
